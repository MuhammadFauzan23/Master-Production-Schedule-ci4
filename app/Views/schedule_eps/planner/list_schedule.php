            ...
            <div class="col-12">
                <div class="card mt-3">
                    <div class="card-header">
                        <div class="row">
                            <div class="col mt-2">
                                <h3 class="card-title">Schedule EPS</h3>
                            </div>
                            <div class="col">
                                <div style="text-align: right;">
                                    ...
                                    <a href="" style="background-color:#E0B0FF" class="btn mr-1" data-toggle="modal" data-target="#release"><i class="nav-icon fas fa-reply-all"></i>&nbsp;Release </a>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <table id="tableView" class="table table-bordered" style="width: 100%;">
                            ...
                            <tbody align="center">
                                ...
                                function tgl_next($hari, $thour, $libur) // $hari = data hari | $thour = total hour
                                {
                                    ...
                                    $first_end = $start_time->setTime(15, 50, 0);
                                ...
                                ?>
                                        <tr>
                                            ...
                                            <td style="<?= $colorB ?>"><?= $sampel ? $form['fgcodeint'] : $idfg[0]; ?></td>
                                            <td style="<?= $colorB ?>"><?= $sampel ? $form['fgcodegen'] : $idfg[1]; ?></td>
                                            ...
                                            <td style="<?= $colorB ?>"><?= number_format($form['op_hours']) ?></td>
                                            ...
                                            <td style="<?= $colorB ?>">
                                                <div class="row">
                                                    <div class="col">
                                                        <?php
                                                        $id = base64_encode(urlencode($form["idform"]));
                                                        $fg =  base64_encode(urlencode($form["idfg"]));
                                                        $stts =  base64_encode(urlencode($form["stts"]));
                                                        ?>
                                                        <a href="<?= base_url() . '/schedule/viewupdate?id=' . $id . '&fg=' . $fg . '&sts=' .  $stts ?>" class="btn btn-warning mb-1 edit_data"><i class="fas fa-edit"></i></a>
                                                        <a href="<?= base_url() . '/schedule/deleteform?id=' .  $id ?>" class="btn btn-danger mb-1 hapus"><i class="far fa-trash-alt"></i> </a>
                                                    </div>
                                                    <?php if ($form['tgl'] != 'NO' && $form['tgl'] != 'YES') { ?>
                                                        <div class="col"><a href="<?= base_url() . '/schedule/view_master_data?id=' . $id ?>" class="btn btn-info mb-1"><?= cek_data_list($form['view_form']) ?></a>
                                                        </div>
                                                    <?php } else { ?>
                                                        <div class="col">
                                                            ...
                                                            <a href="<?= base_url() . '/schedule/view_data_sub?id=' . $id ?>" class="btn btn-info mb-1"><?= cek_data_list($form['view_form']) ?></a>
                                                        </div>
                                                    <?php } ?>
                                                </div>
                                            </td>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control machine" id="machine" name="machine">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="shortage_date" class="form-control" id="shortage_date" required>
                            </div>
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-group">
                                <label for="name">Shortage Date</label>
                                <input type="text" name="shortage_date" class="form-control" id="shortage_date">
                            </div>
        ...
        <div class="modal fade" id="changeMaster<?= $form2['idform'] ?>">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form action="<?= base_url() ?>/schedule/data_master?id=<?= $form2['idform'] . '&master=' . $form2['gabung'] ?>" method="POST">
                        <div class="modal-body">
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group">
...
<script type="text/javascript" async>
        ...
        success: function(response) {
            if (response) {
                response.forEach(function(data) {
                    $('.mouldsize').append(`<option value="${data.id}"> ${data.mould_size} </option>`);
                })
            } else {
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response) {
                response.forEach(function(data) {
                    $('.mouldstatus').append(`<option value="${data.id}"> ${data.mould_name} </option>`);
                })
            } else {
                console.log(response)
            }
    ...
    //JAVASCRIPT MENAMPILKAN DATA FINISHGOOD
    $.ajax({
        type: "get",
    ...
    $('#cav').on('change', function() {
        ...
        // PILIHAN MESIN MENJADI AUTO SELECT KETIKA PEMILIHAN BOM SEDANG BERLANGSUNG --------> (DOKUMENTASI TIDAK DIPAKAI) <--------
        // $.ajax({ 
        //     type: "post",
    ...
    $(document).ready(function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                response.forEach(function(data) {
                    $('#join').append(`<option value="${data.idform}">${data.tgl} || ${data.mc} || ${data.material}</option>`);
                    $('#join_sampel').append(`<option value="${data.idform}">${data.tgl} || ${data.mc} || ${data.material}</option>`);
                    $('.join-sampel').append(`<option value="${data.id}">${data.tgl} || ${data.machine} || ${data.material}</option>`);
                })
            }
        });
    ...
    //JAVASCRIPT PEMILIHAN URUTAN PADA DATA JOIN AUTO/MANUAL SELECT PILIHAN MESIN
    $('#join').on('change', function() {
        ...
        $.ajax({
            type: "post",
            url: "<?= base_url('schedule/getmachinefromMaster') ?>?id=" + joinid,
            success: function(mcn) {
                if (joinid > 0) {
                    $('.machine').html(mcn)
                } else {
                    $.ajax({
                        type: "post",
                        url: "<?= base_url('schedule/getmachine') ?>",
                        success: function(mcn) {
                            $('.machine').html(mcn)
                        }
                    })
    ...
    $('#join').change(function() { //HIDDEN DIV START TIME & LIST NUMBER PADA SCHEDULE
        $('#startedtime').val('')
        $('#urutan').val('')
        let data = $('#join').val()
        if (!data) {
    ...
    $('#join_sampel').change(function() { //HIDDEN DIV START TIME & LIST NUMBER PADA SAMPEL
