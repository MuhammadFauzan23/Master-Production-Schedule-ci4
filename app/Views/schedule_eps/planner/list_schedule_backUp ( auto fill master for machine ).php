                            ...
                            <tbody align="center">
                                ...
                                function tgl_next($hari, $thour, $libur) // $hari = data hari | $thour = total hour
                                {
                                    ...
                                    if ($sisa_menit <= 0) { //Jika kurang dari 0 menit atau tidak lewat jam 16:00 ditampilkan hari itu juga 
                                        ...
                                        if ($sisa_menit <= 530) { //jika kurang dari jam 16:00 / 540 menit
                                ...
                                ?>
                                        <tr>
                                            ...
                                            <td style="<?= $colorB ?>" class="text-center">
                                                <a class="btn btn-secondary" data-toggle="modal" data-target="#remark<?= $form['idform']; ?>"><i class="fab fa-airbnb fa-1x"></i></a>
                                            ...
                                            <td style="<?= $colorB ?>"><?= $form['op_hours'] ?></td>
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <div class="form-group" id="dropdown-machine">
                                <label for="machine">Machine</label>
                                <select class="form-control select2 machine" id="machine" name="machine">
                                    <option value="">-- Select Machine --</option>
                                    <!-- <?php foreach ($mesin as $mc) { ?> 
                                        <option value="<?= $mc['idmachine'] ?>">Machine : <?= $mc['idmachine'] ?> || Desc Machine : <?= $mc['machinedesc'] ?></option>
                                    <?php } ?> -->
                                </select>
                            </div>

                            <div class="form-group" id="input-machine">
                                <label>Machine</label>
                                <div class="input-group">
                                    <input class="form-control machine1" id="machine1" name="machine" readonly></input>
                                </div>
                            </div>
...
<script type="text/javascript" async>
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.length >= 1) {
                    $('#cav').append(`<option value="">-- Select BOM --</option>`);
                    response.forEach(function(data) {
                        $('#cav').append(
                    ...
                    $('#cav').append(`<option value="">-- BOM tidak ada --</option>`);
                    response.forEach(function(data) {})
                    Swal.fire({
                        position: 'center',
                        icon: 'warning',
                        title: 'BOM pada Finishgood tidak ada!',
    ...
    $('#cav').on('change', function() {
        ...
        // machine auto select pada pemilihan BOM tidak jadi dipakai
        // $.ajax({ 
        //     type: "post",
    ...
    $(document).ready(function() {
        ...
        if (!data) {
            ...
            document.getElementById('input-machine').setAttribute('hidden', true)
        }
    ...
    $('#join').on('change', function() {
        ...
        $.ajax({
            type: "post",
            url: "<?= base_url('schedule/getmachinefromMaster1') ?>?id=" + joinid,
            success: function(data) {
                $('#machine1').val(data.replace(/"([^"]+(?="))"/g, '$1'))
            }
        });
    })
    ...
    $('#join').change(function() { //hiddem div startedtime & list number
        ...
        if (!data) {
            ...
            document.getElementById('list-number').setAttribute('hidden', true)
            document.getElementById('dropdown-machine').removeAttribute('hidden', true)
            document.getElementById('input-machine').setAttribute('hidden', true)
        } else {
            ...
            document.getElementById('dropdown-machine').setAttribute('hidden', true)
            document.getElementById('input-machine').removeAttribute('hidden', true)
        }
    })
