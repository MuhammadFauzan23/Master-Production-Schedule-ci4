                            ...
                            <tbody align="center">
                                ...
                                function tgl_next($hari, $thour, $libur) // $hari = data hari | $thour = total hour
                                {
                                    ...
                                    if ($sisa_menit <= 0) { //Jika kurang dari 0 menit atau tidak lewat jam 16:00 ditampilkan hari itu juga 
                                        ...
                                        } else { //Jika lebih dari jam 16:00 lanjut lagi di keesokan harinya  
                                            while ($sisa_menit > 540) { //jika ke 2nd masih lebih dari 16:00 maka ditambah beberapa hari sesuai sisa jam.
                                                $sisa_menit -= 540;
                                                $date->modify('+1 weekday');
                                            }
                                            $d = $date->format('d-m-Y'); //jika ada hari libur nasional pada proses penambahan hari maka otomatis nambah 1 hari
                                            $sisa_menit += 7 * 60;
                                            while (in_array($d, $libur)) { //proses pengecekan hari libur nasional disaat pertambahan tiap hari
                                                $d = $date->modify('+1 weekday')->format('d-m-Y');
                                            }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.length >= 1) {
                    ...
                    response.forEach(function(data) {
                        $('#cav').append(
                            `<option value="${data.ups}:${data.idmaterial}:${data.stdpcs}:${data.cycletime}:${data.idxlen}:${data.revno}">BOM : ${data.ups} |---| Rev No : ${data.revno} |---| Material : ${data.idmaterial}</option>`);
                    })
                } else {
